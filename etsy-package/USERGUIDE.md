...
├── TalkingRobot-v*/            📦 Build staging dir (rebuilt on demand)
├── TalkingRobot-v*.zip         📦 Final ZIP (this is what the buyer downloads)
│
...
- **What:** Node script that renders the 6 printable HTML templates to
  ...
  everything into `TalkingRobot-v<version>.zip`.
...
- **Outputs:**
  ...
  - `etsy-package/TalkingRobot-v<version>/` (staging dir, rebuilt)
  - `etsy-package/TalkingRobot-v<version>.zip` (the buyer ZIP)
...
- **What:** "We Chat with AI Robot Friends!" poster in 5 big kid-safe steps.
...
- **The seven mockups:**
  ...
  2. **16-cell feature grid** — 12 personalities × features
...
11. Attach the digital file: `TalkingRobot-v<version>.zip`.
...
17. Keep old ZIPs tracked in Git (the `.gitignore` exemption keeps
    `etsy-package/TalkingRobot-v*.zip` in history) so you can always
