 ...
 * micro:bit sensor sessions (Choose a personality, tilt, emotion, etc.).
 *
 ...
 *
 * Each CSV matches the format produced by the app's Chat panel CSV export:
 ...
 *   - emotion-walk-north     (heading ~10° drifting)
...
const SCENARIOS = {
  ...
  'emotion-walk-north': ({ duration_s = 60, hz = 5 }) => {
    const out = [];
    let h = 8 + gauss(0, 3);
    for (let i = 0; i < duration_s * hz; i++) {
      const t = i * (1000 / hz);
      ...
      if (h < 0) h += 360;
      if (h >= 360) h -= 360;
      out.push([t, 'emotion', +h.toFixed(1)]);
    }
    return out;
  },
...
const plan = [
  ...
  { scenario: 'emotion-walk-north', n: 9 },
...
for (const { scenario, n } of plan) {
  for (let i = 1; i <= n; i++) {
    ...
    if (scenario === '5-second-bursts') {
      extras.sensor = ['temp', 'light', 'sound', 'emotion'][i % 4];
    }
...
\`Graph\` tab of ${'Talking Robot'} via its "Load CSV" button.
...
| \`emotion-walk-north\` | ${plan.find(p => p.scenario === 'emotion-walk-north').n} | Circular data, drift analysis |
