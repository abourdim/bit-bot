 ...
 *
 * Produces:
 *   etsy-package/TalkingRobot-v2.0.0/          (folder)
 *   etsy-package/TalkingRobot-v2.0.0.zip       (final ZIP)
...
const ZIP_DIR = resolve(PKG, `TalkingRobot-${VERSION}`);
...
async function main() {
  console.log(`\n🎮 Talking Robot Etsy Package Builder — ${VERSION}\n`);
  ...
  const zipPath = join(PKG, `TalkingRobot-${VERSION}.zip`);
  if (existsSync(zipPath)) rmSync(zipPath);
  let zipped = false;
  try { execSync(`cd "${PKG}" && zip -r "${zipPath}" "TalkingRobot-${VERSION}/"`, { stdio: 'pipe' }); zipped = true; } catch {}
  if (!zipped) try { execSync(`cd "${PKG}" && ditto -c -k --keepParent "TalkingRobot-${VERSION}" "${zipPath}"`, { stdio: 'pipe' }); zipped = true; } catch {}
